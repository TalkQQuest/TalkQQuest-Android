...
피그마 와이어프레임에는 화면 크기 프레임이 아주 많지만(초기 시안 54개 → **최신 시안 87개**), 이걸 그대로 화면 수만큼 만들지 않습니다.
...
이 정리를 거쳐 초기 시안 기준 **논리 화면 35개 + 팝업 4개**로 집계했습니다(최신 시안 기준 재집계는 아직). 화면별 "와이어프레임 대응" 관계는 [`NAVIGATION.md`](NAVIGATION.md)에 정리돼 있으니, 내 담당 화면이 어떤 프레임에서 왔는지 거기서 확인하세요.
...
> **값의 최종 출처(source of truth)는 디자이너가 넘긴 최신 UI CSS 추출본(2026-07-18)입니다.** 아래 표와 다르면 항상 최신 추출본이 우선이고, 화면을 만들 땐 그 화면의 프레임 CSS를 직접 열어 값을 그대로 옮겨 적으세요(눈대중 재구성 금지). 초기에 받은 `design/design-system.css` / `design/components.css`는 초기 기준이라 최신과 어긋난 값이 있습니다. (자세한 사용법은 팀 배포용 `공통 세팅 안내.md` 참고)
