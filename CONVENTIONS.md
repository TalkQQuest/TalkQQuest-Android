...
- **앱은 세로 고정입니다.** `AndroidManifest.xml`의 MainActivity에 `screenOrientation="portrait"`가 걸려 있어, 축소율은 항상 세로 기준으로만 계산됩니다. 가로 레이아웃은 만들지 않습니다. (단 targetSdk 36 정책상 화면 폭 600dp 이상 기기에서는 이 고정이 무시됩니다.)
