...
| --- | --- | --- | --- |
| A | 지니/전준호 | 진입(스플래시·로그인·회원가입·온보딩), 아카이브, 프로필 | 17 |
| B | 이도/윤기수 | 미션(홈·미션상세·진입·북마크), AI 대화(대화진행·완료·XP·피드백) | 9 |
| C | 훈/김재훈 | 커뮤니티(모임 검색·생성·참여·내 모임), 성장 리포트(월간·주간 비교) | 9 |

> 화면 수는 **논리 화면 기준 총 35개 + 팝업 4개**입니다. 피그마 와이어프레임에는 화면 크기 프레임이 54개 있지만, 같은 화면의 상태 변형(예: 미션 상세 2개, 대화 진행 2개)은 하나의 화면으로 구현하므로 실제 구현 대상은 35개입니다. 자세한 매핑은 [`NAVIGATION.md`](NAVIGATION.md) 참고.
