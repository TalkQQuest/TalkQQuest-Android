...
> 화면 수는 UI 5차 기준 집계값입니다(전체화면 프레임 54개 → 논리 화면 35개 + 팝업 4개). 같은 화면의 상태 변형(예: 미션 상세 2개, 대화 진행 2개)은 하나의 화면으로 구현하기 때문에 프레임 수보다 적습니다. **현재 디자인 기준은 UI 7차이고 전체화면 프레임은 87개로 늘었으며, 논리 화면 재집계는 아직입니다.** 자세한 매핑은 [`NAVIGATION.md`](NAVIGATION.md) 참고.
>
